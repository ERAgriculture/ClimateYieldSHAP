...
local_CalcClimate<-function(DATA,
                      CLIMATE,
                      ID,
                      Rain.Data.Name,
                      Temp.Data.Name,
  ...
  DATA <- SS$DATA
  SS <- SS$SS
  MCode.SS <- apply(SS[, c(..ID, "EU", "P.Date.Merge", "M.Year")], 
                    1, paste, collapse = "_")
  ...
  if (nrow(SS) > 0) {
    CLIMATE <- CLIMATE[, !c("Latitude", "Longitude", "Buffer", 
                            "DayCount")]
    ...
    B <- lapply(Sites, FUN = function(Site) {
      cat("\r                                                                                                                                          ")
      cat("\r", paste0(Temp.Data.Name, " x ", Rain.Data.Name, 
                       ": Estimating seasonal climate for  site: ", 
                       match(Site, Sites), "/", length(Sites)))
      flush.console()
      SS.N <- SS[which(SS[, ..ID] == Site)][is.na(M.Year.Code), 
                                            `:=`(M.Year.Code, "")]
      ...
      if (is.null(Climate)) {
        ...
        A <- rbindlist(lapply(1:nrow(SS.N), FUN = function(i) {
          # MODIFICATION 1: Add "PreSowing" window from 30 days before planting to planting day
          WINS <- rbind(data.table(Name = c("Data", 
                                            "EcoCrop", "PreSowing"), 
                                   Start = c(rep(Win.Start, 2), -30), 
                                   End = c(SS.N[i, SLen.Merge], SS.N[i, SLen.EcoCrop], 0)), 
                        Windows)
        ...
        if(Do.LT.Avg){
          ...
          LT.A<-lapply(1:nrow(EU.N.S),FUN=function(i){
            ...
            WINS <- rbind(data.table(Name = c("Data", 
                                              "EcoCrop", "PreSowing"), 
                                     Start = c(rep(Win.Start, 2), -30), 
                                     End = c(SS.N[i, SLen.Merge][1], SS.N[i, SLen.EcoCrop][1], 0)), 
                          Windows)[, `:=`(End, round(End, 0))][, `:=`(Start, round(Start, 0))]
