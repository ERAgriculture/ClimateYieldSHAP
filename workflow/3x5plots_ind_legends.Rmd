---
title: "Understanding the ERA SHAP Beeswarm Grid Visualization Script"
...
This document explains the R script used to create a grid of SHAP (SHapley Additive exPlanations) beeswarm plots for the ERA (Evidence for Resilient Agriculture) project. The script generates a comprehensive visualization showing how climate variables affect crop yields across different Warm Agro-Ecological Zones (AEZs) in Africa. A parallel script follows the same approach for Cool AEZs.
...
- Columns 2-5 show SHAP beeswarm plots for the most important climate variables
...
The script begins by defining the experiment name and selecting the "Warm" AEZ temperature regime. It also specifies the type of SHAP plot to use (`GAMCI_shap_plot_` uses Generalized Additive Models to fit confidence bands).
...
This section loads the model performance metrics (R² values) and the number of observations for each AEZ. These metrics will be displayed on the feature importance plots to provide context for the model's predictive power.
...
This code loads the feature importance plots for each AEZ from previously saved RDS files. These plots show which climate variables have the strongest influence on yield predictions in each zone. The feature names are also made more readable by replacing underscores with spaces.

# Defining SHAP Plot Paths

The script then defines the paths to the SHAP beeswarm plots for each AEZ and feature combination. For each AEZ (Semiarid, Subhumid, Humid), it selects four key climate variables and pairs them with interaction variables.
...
Similar code blocks define paths for rows 2 (Subhumid) and 3 (Humid). For each feature pair, the script also creates a human-readable title using the `spell_out_features()` function.
...
Finally, the script saves the combined visualization as a high-resolution PNG file. The filename includes the plot type and temperature regime.

# Understanding the Visualization

The final grid visualization provides multiple layers of information:
...
This enables researchers to see how climate-yield relationships change across moisture gradients.

# Parallel Script for Cool AEZs

There is a similar script that follows the same approach to create an equivalent visualization for the Cool AEZs (Cool Semiarid, Cool Subhumid, Cool Humid). Together, these visualizations provide a comprehensive view of climate-yield relationships across the major agro-ecological zones of Africa.

# Conclusion

This visualization script creates a powerful tool for understanding how climate variables affect crop yields across different agro-ecological zones. By combining feature importance plots with SHAP beeswarm plots arranged by AEZ, it enables researchers to:

1. Identify the most important climate drivers of yield in each region
2. Understand how these drivers' effects vary with their magnitude
3. Detect important interaction effects between climate variables
4. Compare patterns across different ecological zones

The resulting visualization serves as both an analytical tool for researchers and a communication tool for stakeholders working on agricultural resilience in a changing climate.