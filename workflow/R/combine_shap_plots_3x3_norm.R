# Combine SHAP plots in a 3 by 3 grid while using normalized legends
...
combine_shap_plots_3x3 <- function(shap_plots, rainfall_plots, temp_plots, main_title, AEZ_temp, row_list, rainfall_min_max, temperature_min_max) {
  ...
  rainfall_plots <- lapply(seq_along(rainfall_plots), function(i) {
    ...
    if (i == 3) {
      norm_min <- rainfall_min_max[1]
      norm_max <- rainfall_min_max[2]
      cleaned_plot <- cleaned_plot + 
        theme(
          labels = function(x) scales::rescale(x, to = c(norm_min, norm_max)),
          legend.position = "bottom",
          legend.direction = "horizontal",
          legend.key.width = unit(1.5, "cm"),
          legend.key.height = unit(0.3, "cm"),
  ...
  temp_plots <- lapply(seq_along(temp_plots), function(i) {
    ...
    if (i == 3) {
      norm_min <- temperature_min_max[1]
      norm_max <- temperature_min_max[2]
      cleaned_plot <- cleaned_plot + 
        theme(
          labels = function(x) scales::rescale(x, to = c(norm_min, norm_max)),
          legend.position = "bottom",
