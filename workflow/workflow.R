...
if (is.na(AEZ)){
  AEZ <- NULL
  AEZ <- "Cool.Semiarid"  # Set to NULL for the entirity of Africa
...
for (model_i in 1:length(config$models)){
  ...
  end_time <- Sys.time()
  cat("Model training and testing time:", difftime(end_time, start_time, units = "secs"), "seconds\n")
    ...
    for (main_feature in feature_names) {
      for (secondary_feature in feature_names) {
        if (main_feature != secondary_feature) {
          cat("\nEstimating SHAP for one feature...\n")
          start_time <- Sys.time()
          ...
          end_time <- Sys.time()
          cat("SHAP estimation time for one feature:", difftime(end_time, start_time, units = "secs"), "seconds\n")
